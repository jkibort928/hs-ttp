...
parseArgs strs = helper strs [] [] [] [] [] true
	where
		helper args argv fs lfs fargs lfargs processFlags = case args of
			...
			(s:[])
				| processFlags && isLFlag s && isArgLFlag s 	-> throw (Error "Error: A LongFlag is missing an argument!")
				| processFlags && isLFlag s 					-> 				(argv,		fs,						(removeDashes s):lfs),		fargs,		lfargs
				| processFlags && isFlag s && isArgFlag s 		-> throw (Error "Error: A Flag is missing an argument!")
